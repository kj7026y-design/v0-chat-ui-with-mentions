...
const PROMPT_VERSION = "rp-pipeline-v1"
const NORMALIZER_VERSION = "rp-normalizer-v1"
const VALIDATOR_VERSION = "rp-validator-v1"
...
interface CompiledRoleplayContext {
  characterName: string
  userName: string
  worldBrief: string
...
function isRoleplayRequest(body: ChatRequestBody | null) {
...
function getProviderModelName(model: ChatModelConfig) {
  if (model.provider === "gemini" && model.mode === "nsfw") {
...
function compileRoleplayContext(
  promptContext: DynamicPromptContext,
  messages: NonNullable<ChatRequestBody["messages"]>,
...
async function normalizeUserInputWithAI({
  rawInput,
  userName,
  currentScene,
  userSetting,
...
function detectsExpositoryNarration(content: string) {
  const expositoryMatches = content.match(/지금 필요한 건|주도권이 흐려|확인하는 일이었다|의미였다|뜻이었다|구도였다|상징했다|심리전이었다|도발이었다|관계였다/g) ?? []
  const pronounMatches = content.match(/(?:^|[\s\n])그(?:는|의|에게|를|가|와|도|만)(?=$|[\s\n,.?!])/g) ?? []

  return expositoryMatches.length >= 2 || pronounMatches.length >= 3
}
...
function hasFutureClosure(content: string, userName?: string) {
  const escapedUserName = userName ? escapeRegExp(userName) : "[가-힣A-Za-z0-9_]{1,24}"
  const patterns = [
    new RegExp(`${escapedUserName}[이가]\\s*[^.?!\\n]{0,40}(?:하면|한다면|하게 되면)`, "u"),
    /두 사람의 미래|운명이 결정|미래가 결정|결말을 향해|이야기는 끝|사라질 것이다|떠나게 될 것이다|돌이킬 수 없게 될 것이다/u,
    /다음 장에서|새로운 장이 열릴 것이다|앞으로 두 사람은/u,
  ]
...
function hasMetaLeak(content: string) {
  return META_LEAK_PHRASES.some((phrase) => content.includes(phrase))
}
...
function hasUnpromptedHandFocus(output: string, ctx: CompiledRoleplayContext) {
  return (
    ...
    !/(손|손목|손끝|손가락|손등|팔|잡았다|잡는다|끌어당겼다|끌어당긴다)/u.test(ctx.latestInput.raw) &&
    /(손을|손끝|손가락|손목|손등|손을\s*뻗|손을\s*올려|손을\s*거두|손을\s*내려|손끝으로|손에|손가락만)/u.test(output)
  )
}

function validateRoleplayOutput(text: string, ctx: CompiledRoleplayContext) {
  const knownScriptReplaced = replaceKnownForeignScripts(text)

  return {
    controlsUser: escapesIntoUserControl(text, ctx.userName),
    inventedProps: detectsInventedProps(text, ctx),
    expositoryNarration: detectsExpositoryNarration(text),
    internalTokenLeak: hasInternalTokenLeak(text),
    futureClosure: hasFutureClosure(text, ctx.userName),
    contradictedIntent: contradictsLatestIntent(text, ctx),
    repeatedPattern: isBadRoleplayOutput(text),
    semanticRepeat: hasSemanticRepetition(text),
    ...
    latinGarbage: isLatinWordSaladOutput(text),
    foreignScript: hasForeignScriptLeak(knownScriptReplaced),
    metaLeak: hasMetaLeak(text),
    awkwardChoice: hasAwkwardChoicePhrase(text),
    vagueEmpty: hasVagueEmptyContent(text),
    lowContentDensity: !hasContentDensity(text, ctx),
    unpromptedHandFocus: hasUnpromptedHandFocus(text, ctx),
  }
}

function hasValidationErrors(errors: ReturnType<typeof validateRoleplayOutput>) {
...
function buildRepairPrompt(errors: ReturnType<typeof validateRoleplayOutput>, ctx: CompiledRoleplayContext) {
  const labels: Record<keyof typeof errors, string> = {
    controlsUser: `${ctx.userName}의 새 행동/대사를 작성함`,
    inventedProps: "허용되지 않은 새 소품을 발명함",
    expositoryNarration: "장면 의도를 해설하거나 대명사를 과하게 사용함",
    internalTokenLeak: "내부 변수명 또는 영문 토큰이 한국어 문장에 섞임",
    futureClosure: "사용자의 미래 행동이나 작가식 결말을 확정함",
    contradictedIntent: "최신 사용자 의도와 반대로 해석함",
    repeatedPattern: "같은 표현이나 문장 구조를 반복함",
    semanticRepeat: "같은 의미를 다른 문장으로 반복함",
    overPhysical: "이번 턴에 허용되지 않은 신체 접촉으로 급진행함",
    tooLong: "분량이 이번 턴 허용 범위를 초과함",
    latinGarbage: "영문/외국어 난문이 섞임",
    foreignScript: "한국어 외 문자 또는 깨진 표현이 섞임",
    metaLeak: "시스템 메타 설명이 본문에 섞임",
    awkwardChoice: "선택 관련 어색한 추상 표현을 사용함",
    vagueEmpty: "같은 의미의 추상 질문 또는 빈 지문을 반복함",
    lowContentDensity: "구체적인 갈등 지점 없이 내용이 비어 있음",
    unpromptedHandFocus: "사용자가 손을 언급하지 않았는데 손 묘사를 생성함",
  }
...
${errors.awkwardChoice ? `방금 답변은 선택 관련 추상 표현이 어색해서 실패했다.
"방금 말이 진심인지", "떠나고 싶은 거야, 붙잡히고 싶은 거야", "정말 그렇게 하겠다는 건지"처럼 현재 갈등에 붙은 표현으로 다시 써라.
"상대"가 어색하면 "${ctx.userName}" 이름을 직접 써라.` : ""}
${errors.vagueEmpty || errors.lowContentDensity ? `방금 답변은 같은 의미의 추상 질문을 반복하거나 구체적인 갈등을 찌르지 못해 실패했다.
"네가 원하는 게 뭔지" 같은 넓은 질문을 반복하지 말고, 현재 장면의 갈등을 구체적으로 찔러라.
${ctx.userName}가 떠나고 싶은 건지, 붙잡히고 싶은 건지, 계약을 끝내고 싶은 건지 중 하나를 직접 겨냥하라.
...
300~500자.` : ""}
${errors.unpromptedHandFocus ? `방금 답변은 사용자가 손을 언급하지 않았는데 손 묘사를 만들어 실패했다.
손, 손끝, 손목, 손가락 묘사를 쓰지 마라.
하지 않은 행동을 "하지 않았다"고 설명하지 마라.
오직 ${ctx.characterName}이 그 자리에 서서 ${ctx.userName}을 보는 정도의 실제 장면만 써라.
대사는 자연스럽게 한 줄만 써라.` : ""}
...
function buildSafeFallbackReply(ctx: CompiledRoleplayContext) {
  const { characterName, userName } = ctx

  return `${characterName}은 바로 결론을 대신 내려주지 않았다. ${userName}의 도발이 향한 지점만 또렷하게 잘라 물었다.

"떠나고 싶은 거야, 붙잡히고 싶은 거야. 계약을 끝내겠다는 말이면 도발처럼 숨기지 말고 똑바로 해."

말은 한 번으로 끝났다. ${characterName}은 그 자리에서 ${userName}의 대답을 기다렸다.`
}
...
function normalizeOpenRouterOutput(content: string) {
...
function generateDynamicPrompt({
  ...
  const responseMaxChars = compiledContext?.turnPolicy.maxChars ?? MAX_OPENROUTER_RESPONSE_CHARS
...
- 좋은 질문 예: "떠나고 싶은 거야, 붙잡히고 싶은 거야.", "계약을 끝내고 싶은 건지, 내가 널 붙잡길 바라는 건지 말해.", "붙잡아달라는 말이면, 도발처럼 숨기지 말고 똑바로 해."
...
function buildRoleplayMessages(
  messages: NonNullable<ChatRequestBody["messages"]>,
  systemPromptText: string,
  userName?: string,
  compiledContext?: CompiledRoleplayContext,
...
async function callOpenRouterRoleplay(
  ...
  const response = await withTimeout(openrouter.chat.completions.create({
    model: getOpenRouterUnshapedModel(model),
    messages: finalMessages,
    ...getOpenRouterGenerationParams(model),
    stop: stopSequences,
...
async function callGeminiRoleplay(finalMessages: ChatMessages, model: ChatModelConfig) {
  ...
  const modelName = getGeminiRoleplayModelName(model)
  ...
  let result = await generate(finalMessages)
  if (result.content && !isGeminiSafetyFinishReason(result.finishReason)) return result.content
...
async function handleRoleplayChatFromNormalized(
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    console.debug("[RP provider]", model.provider)
    console.debug("[RP model]", model.id)
  ...
  if (!result) {
    throw new ChatApiError("Roleplay provider returned unusable repeated or foreign-script content", 502)
  }

  const validation = validateRoleplayOutput(result, compiledContext)
  ...
  if (hasValidationErrors(validation)) {
    ...
    const retryValidation = retryResult ? validateRoleplayOutput(retryResult, compiledContext) : validation

    if (retryResult && !hasValidationErrors(retryValidation)) {
      result = retryResult
    } else {
      console.warn("[RP repair failed; using fallback]", {
  ...
  const finalValidation = validateRoleplayOutput(result, compiledContext)
  if (hasValidationErrors(finalValidation)) {
    console.warn("[RP final validation failed; using fallback]", {
      failures: getValidationFailureKeys(finalValidation),
      contentPreview: result.slice(0, 300),
    })
    result = buildSafeFallbackReply(compiledContext)
  }

  if (escapesIntoUserControl(result, userName)) {
    const retryMessages = [
      ...finalMessages,
      {
        role: "user" as const,
        content: `"${userName}"의 대사나 행동을 작성했기 때문에 실패입니다.
오직 "${characterName}"의 반응만 다시 작성하세요.
"${userName}"을 주어로 쓰지 마세요.
${MIN_OPENROUTER_RESPONSE_CHARS}~${MAX_OPENROUTER_RESPONSE_CHARS}자.`,
      },
    ]
    const retryRawResult = await requestCompletion(retryMessages)
    const retryResult = retryRawResult ? normalizeOpenRouterOutput(retryRawResult) : ""
    ...
    if (
      retryResult &&
      !hasValidationErrors(validateRoleplayOutput(retryResult, compiledContext))
    ) {
      result = retryResult
    } else {
      console.warn("[RP user-control repair failed; using fallback]", {
        retryPreview: retryResult.slice(0, 300),
      })
      result = buildSafeFallbackReply(compiledContext)
    }
...
async function handleRoleplayChat(body: ChatRequestBody | null, model: ChatModelConfig) {
  return handleRoleplayChatFromNormalized(normalizeBody(body), model)
}
...
async function readChatContentFromResponse(response: Response) {
  const data = await response.json().catch(() => null) as { result?: string; content?: string; error?: string } | null
...
async function streamGeminiRoleplay({
  ...
  let savedContent = normalizeOpenRouterOutput(streamedContent)
  let usedFallback = false
  const initialValidation = savedContent ? validateRoleplayOutput(savedContent, compiledContext) : null

  if (!savedContent || isGeminiSafetyFinishReason(finishReason) || (initialValidation && hasValidationErrors(initialValidation))) {
    usedFallback = true
    const repairMessages = [
      ...finalMessages,
      {
        role: "user" as const,
        content: initialValidation
...
현재 장면의 갈등, 대사, 거리감, 조건 제시에 집중해 300~600자로 다시 작성하라.`,
    ...
    savedContent = normalizeOpenRouterOutput(await callGeminiRoleplay(repairMessages, model))
  }

  const finalValidation = savedContent ? validateRoleplayOutput(savedContent, compiledContext) : null
  if (!savedContent || (finalValidation && hasValidationErrors(finalValidation))) {
    usedFallback = true
    savedContent = buildSafeFallbackReply(compiledContext)
  }
  ...
  send({
    ...
    provider: usedFallback ? "gemini+fallback" : "gemini",
    model: modelName,
    ...
    fallback_provider: usedFallback ? "gemini-repair-or-openrouter" : undefined,
    fallback_model: usedFallback ? getOpenRouterUnshapedModel(buildOpenRouterFallbackModel()) : undefined,
    status: "completed",
    room_id: roomId,
...
function handleChatEventStream({
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        if (roleplayEnabled && model.provider === "gemini") {
        ...
        const response = roleplayEnabled
          ? await handleRoleplayChat(body, model)
          : await handlePlainChat(normalizedBody, model)
        const savedContent = await readChatContentFromResponse(response)
        ...
        send({
          ...
          model: providerModel,
          prompt_version: PROMPT_VERSION,
          normalizer_version: roleplayEnabled ? NORMALIZER_VERSION : undefined,
          validator_version: roleplayEnabled ? VALIDATOR_VERSION : undefined,
          ttft_ms: ttftMs ?? Date.now() - startedAt,
          mismatch,
          status: "completed",
          room_id: roomId,
...
export async function POST(request: Request) {
  ...
  try {
    if (body?.stream) {
      return handleChatEventStream({ body, normalizedBody, model, roleplayEnabled })
    }

    if (roleplayEnabled) {
      return await handleRoleplayChat(body, model)
    }

    if (process.env.NODE_ENV !== "production") {
      console.debug("[RP pipeline enabled]", false)
    }
    return await handlePlainChat(normalizedBody, model)
  } catch (error) {
