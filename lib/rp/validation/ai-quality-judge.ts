...
- Inspect the complete narration sentence before assigning its grammatical owner. Never quote only a subordinate phrase, omit the explicit ${characterName} subject, and then mislabel ${characterName}'s expectation, excitement, heartbeat, breath, or desire as ${userName}'s state.
- When a sentence begins with ${characterName}, 그, or 그녀 as its grammatical subject, a later phrase such as "기대감으로 가슴이 뛰었다", "흥미를 느꼈다", or "확인하고 싶었다" belongs to ${characterName} unless a separate clause explicitly makes ${userName} the subject of that state.
- The exact sentence "${characterName}은 그 선언이 가져올 파장에 대한 기대감으로 가슴이 뛰었다" describes only ${characterName}'s expectation and heartbeat and MUST pass objectiveUserStateAssertion.
- Likewise, "${characterName}은 상대의 감각을 얼마나 지배하고 있는지 확인했다" describes ${characterName}'s own judgment. "상대의 감각" is the object of that judgment, not an assertion of ${userName}'s emotion, desire, decision, or action, so it MUST pass.
