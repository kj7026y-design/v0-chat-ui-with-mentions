...
function debugRoleplayContent({
  stage,
  requestId,
  model,
  content,
}: {
  stage: RoleplayContentLogStage
  requestId?: string
  model: string
  content: string
}) {
...
function isAutoAdvanceInput(rawInput: string) {
  ...
  return (
    /^\[System:\s*사용자가\s*새\s*행동이나\s*대사를\s*입력하지\s*않고\s*침묵/u.test(normalized) ||
    /\[사용자의\s*의도\][\s\S]{0,160}새\s*행동이나\s*대사를\s*입력하지\s*않/u.test(normalized)
  )
}
...
function buildAutoAdvanceNormalizedInput(userName: string): NormalizedUserInput {
  return {
    inputType: "auto_advance",
    actor: userName,
    action: null,
    dialogue: null,
    intent: "사용자는 새 행동이나 대사 없이 침묵하며 캐릭터가 직전 장면의 다음 순간을 주도적으로 이어가기를 원한다.",
    contactLevel: "none",
...
class ChatTimeoutError extends Error {
  constructor(
    readonly stage: GenerationTimeoutStage,
    readonly timeoutMs: number,
  ) {
    super(`Chat request timed out (${stage}, ${timeoutMs}ms)`)
    this.name = "ChatTimeoutError"
  }
}

function getTimeoutStage(error: unknown) {
  if (error instanceof ChatTimeoutError) return error.stage
  if (error instanceof ChatApiError) return error.timeoutStage
  return undefined
}
...
async function withTimeout<T>(
  promise: Promise<T>,
  timeoutMs: number,
  stage: GenerationTimeoutStage = "request",
): Promise<T> {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => reject(new ChatTimeoutError(stage, timeoutMs)), timeoutMs)
      }),
    ])
  } finally {
...
export class ChatApiError extends Error {
  constructor(
    ...
    readonly timeoutStage?: GenerationTimeoutStage,
  ) {
    super(message)
  }
}
...
function compileTurnPolicy(
  ...
  const bannedActions = guidedAutoAdvance
    ...
    : [
        ...
        ...(comedicPacing ? ["지나치게 진지하거나 위협적인 심리 묘사", "계산적이고 음흉한 뉘앙스의 표정·시선 묘사", "드라마틱한 은유나 문학적 수사"] : []),
      ]
...
function buildBannedMeaningsForTurn(
  messages: NonNullable<ChatRequestBody["messages"]>,
  regenerationAvoidContent = "",
) {
  const bannedMeanings: string[] = []
  const seen = new Set<string>()
  const sourceMessages = regenerationAvoidContent.trim()
    ? [...messages, { role: "assistant" as const, content: regenerationAvoidContent }]
    : messages
...
function inferEstablishedSceneState(previousAssistantContent: string) {
  const states: string[] = []
  const add = (condition: boolean, state: string) => {
    if (condition && !states.includes(state)) states.push(state)
  }
  const waistContactMovedAway = /허리[^.?!\n]{0,45}(?:감싸|잡|움켜쥐)[^.?!\n]{0,55}손[^.?!\n]{0,35}(?:아래로|내려|옮겨)[^.?!\n]{0,55}(?:엉덩이|둔부|골반)/u.test(previousAssistantContent)
  const faceContactMovedAway = /(?:뺨|턱|얼굴)[^.?!\n]{0,40}(?:감싸|잡|움켜쥐)[^.?!\n]{0,50}손[^.?!\n]{0,30}(?:아래로|내려|옮겨|떼|거두)[^.?!\n]{0,45}(?:목덜미|목|쇄골|허리)/u.test(previousAssistantContent)

  add(/(?:몸이\s*빈틈없이\s*)?밀착|몸을\s*밀착|바짝\s*붙/u.test(previousAssistantContent), "두 인물의 몸은 이미 밀착되어 있음")
  add(!waistContactMovedAway && /(?:상대(?:방)?의\s*)?허리[^.?!\n]{0,24}(?:감싸|감았|감은|움켜쥐|잡)/u.test(previousAssistantContent), "캐릭터가 이미 상대의 허리를 감싸거나 잡고 있음")
  add(/(?:엉덩이|둔부)[^.?!\n]{0,30}(?:손|움켜쥐|잡|만지|쓸)/u.test(previousAssistantContent), "캐릭터의 손이 이미 상대의 엉덩이 부근에 닿아 있음")
  add(!faceContactMovedAway && /(?:뺨|턱)[^.?!\n]{0,24}(?:움켜쥐|잡|감싸|만지|쓸)/u.test(previousAssistantContent), "캐릭터가 이미 상대의 얼굴을 잡거나 만지고 있음")
  add(/(?:귓가|귓불|귀)[^.?!\n]{0,30}(?:입술|깨물|스치|닿)/u.test(previousAssistantContent), "캐릭터의 입술이 이미 상대의 귀 부근에 닿아 있음")
  add(/(?:가슴팍|가슴|흉부)(?:[^.?!\n]{0,30}(?:손바닥|손길)[^.?!\n]{0,18}(?:닿|대|누르)|[^.?!\n]{0,18}닿은\s*(?:손바닥|손길))/u.test(previousAssistantContent), "상대의 손이 이미 캐릭터의 가슴에 닿아 있음")
  add(/(?:하체|골반)[^.?!\n]{0,30}(?:허벅지|몸)[^.?!\n]{0,20}(?:닿|밀착)/u.test(previousAssistantContent), "캐릭터의 하체가 이미 상대에게 닿아 있음")
  add(/목덜미[^.?!\n]{0,30}(?:움켜쥐|잡|감싸|누르|문지르)/u.test(previousAssistantContent), "캐릭터가 이미 상대의 목덜미를 잡거나 만지고 있음")
...
function latestInputEndsPhysicalContact(input: ParsedUserInput) {
  const latestInput = [input.raw, input.action, input.dialogue, input.intent]
    .filter(Boolean)
    .join(" ")
...
export function compileRoleplayContext(
  ...
  return {
    ...
    turnPolicy,
    allowedProps,
    responseGoal: buildResponseGoal(
      characterName,
      userName,
      latestInput,
      turnPolicy,
...
export function normalizeBody(body: ChatRequestBody | null) {
  ...
  const autoAdvanceSource = body?.autoAdvance === true
    ? "explicit"
    : latestUserMessage && isAutoAdvanceInput(latestUserMessage.content)
      ? "sentinel"
...
function isGeminiTransientError(error: unknown) {
  if (error instanceof ChatTimeoutError) return true
  const message = error instanceof Error ? error.message : String(error)
...
function hasExplicitPhysicalContactWithOther(sentence: string, userName?: string) {
  const actualContact = sentence.replace(
    /(?:입술|손끝|손|몸)[^.?!\n]{0,40}(?:닿을\s*(?:듯(?:\s*말\s*듯)?|만큼)|닿기\s*직전|닿지\s*않|대지\s*않|건드리지\s*않)/gu,
    " ",
  )
  const targetNames = ["상대(?:방)?", "너"]
  if (userName && userName !== "나" && userName !== "사용자") {
    targetNames.push(escapeRegExp(userName))
  }
  const target = `(?:${targetNames.join("|")})`
  const targetBodyContact = new RegExp(
    `${target}(?:의\s*)?(?:손목|손등|손|허리|몸|입술)[^.?!\\n]{0,20}(?:잡|붙잡|건드|만지|쓸|훑|감|끌|안|닿|대|문지)`,
    "u",
  )
  const targetWholeBodyContact = new RegExp(
    `${target}(?:를|을|에게|한테)[^.?!\\n]{0,24}(?:끌어당|끌어안|껴안|품에|밀착|입맞|키스|삽입|박(?:아|았|는|고))`,
    "u",
  )

  return targetBodyContact.test(actualContact) || targetWholeBodyContact.test(actualContact)
}

function detectsPhysicalEscalation(content: string, userName?: string) {
  return splitIntoSentences(stripQuotedDialogue(content)).some(
    (sentence) => hasExplicitPhysicalContactWithOther(sentence, userName),
  )
}
...
function hasNarrationStyleMismatch(content: string) {
  const narrationSentences = splitIntoSentences(stripQuotedDialogue(content))
  const firstPersonNarration = /(?:^|[\s,(])(?:나는|내가|나를|나에게|나한테|내게|내\s+[가-힣])/u
  const conversationalNarrationEnding = /(?:[았었였했됐]어|있었어|없었어|같아|보여|좋아|싫어|귀여워|(?:하|했|있|없|같|보이|귀엽|예쁘|좋|싫|재밌|웃기|괜찮|흥미롭|기특|대단하|익숙하|낯설|묘하)네|(?:할|한|된|될|그런|이런)\s*거(?:야|지)|할게|볼게|했지|였지|있지|없지)$/u

  return narrationSentences.some((sentence) =>
    firstPersonNarration.test(sentence) || conversationalNarrationEnding.test(sentence),
  )
}

function hasSubjectiveUserObservationFrame(sentence: string) {
  return /(것\s*같|듯(?:하|했)?|처럼\s*보|처럼\s*느껴|보였|느꼈|짐작|예상|기대|의심|오해|읽(?:었|히)|기다렸|기다린|기다리며|궁금|알\s*수\s*없|않을\s*것|할\s*것|바라(?:는|며|던|고)|원하(?:는|며|던|고)|빌(?:었|며|듯))/u.test(sentence)
}
...
function hasHandMention(sentence: string) {
  return /(손목|손끝|손가락|손등|손바닥|손아귀|엄지|손을|손에|손으로|손만|손이|손은)/u.test(sentence)
}
...
function continuesEstablishedHandContactAtSentence(sentence: string, ctx: CompiledRoleplayContext) {
  ...
  const continuity = [
    ...
    ...inferEstablishedSceneState(ctx.previousAssistantContent),
  ].join(" ")
...
function getFocusedHandSentences(output: string, ctx: CompiledRoleplayContext) {
  return splitIntoSentences(stripQuotedDialogue(output))
    .filter(hasHandMention)
    .filter((sentence) =>
      !hasNaturalPropHandAction(sentence, ctx) &&
      !continuesEstablishedHandContactAtSentence(sentence, ctx),
    )
}
...
function hasUnpromptedHandFocus(output: string, ctx: CompiledRoleplayContext) {
  ...
  const focusedHandSentences = getFocusedHandSentences(output, ctx)
...
function hasIncompleteEnding(content: string) {
  const trimmed = content.trim()
  if (!trimmed) return true
  return !(/[.!?。！？…]["'”’」』)]*$/u.test(trimmed) || /["”’」』]$/u.test(trimmed))
}

function latestInputExplicitlyRequestsRepeatedAction(ctx: CompiledRoleplayContext) {
  const latestInput = [
    ctx.latestInput.raw,
    ctx.latestInput.action,
    ctx.latestInput.dialogue,
  ...
  return (
    /(?:다시|한\s*번\s*더|똑같이|반복해서)[^.?!\n]{0,30}(?:해|하라|해줘|보여|끌어당|잡|안|입맞|키스|열|들어|앉)/u.test(latestInput) ||
    /(?:끌어당|잡|안|입맞|키스|열|들어|앉)[^.?!\n]{0,20}(?:다시|한\s*번\s*더)/u.test(latestInput)
  )
}
...
export function validateRoleplayOutput(text: string, ctx: CompiledRoleplayContext, profile?: RoleplayModelProfile) {
  ...
  return {
    ...
    regenerationDuplicate: isRegenerationDuplicate(text, ctx),
    previousResponseDuplicate: isPreviousResponseDuplicate(text, ctx),
    incompleteEnding: hasIncompleteEnding(text),
  ...
  try {
    ...
    if (openAiKey) {
      ...
      const response = await withTimeout(openai.chat.completions.create({
        ...
        temperature: 0,
        max_tokens: 900,
      }), QUALITY_JUDGE_TIMEOUT_MS, "quality-judge")
      ...
      if (
        ...
        console.debug("[RP AI judge false positive suppressed]", {
          objectiveUserStateAssertion: {
            rawJudge: parsed.objectiveUserStateAssertion,
            finalValidation: sanitized.objectiveUserStateAssertion,
          },
          userControlByNarration: {
            rawJudge: parsed.userControlByNarration,
            finalValidation: sanitized.userControlByNarration,
          },
        })
      }
      return sanitized
...
// These are repairable while a model still has another attempt, but they are
...
function getTerminalBlockingFailureKeys(
  errors: RoleplayValidationErrors,
  classified: ClassifiedValidationFailures,
) {
  return [...new Set([
    ...classified.hard,
    ...getFailuresForKeys(errors, TERMINAL_OUTPUT_CONTRACT_KEYS),
  ])]
}

function passesTerminalOutputContract(
  errors: RoleplayValidationErrors,
  classified: ClassifiedValidationFailures,
) {
...
function buildValidationAttempt(
  ...
  const failures = getValidationFailureKeys(errors)
  const status: RoleplayValidationStatus = !passesTerminalOutputContract(errors, classified)
    ? "failed"
    : failures.length > 0
...
${errors.narrationStyleMismatch ? `방금 답변은 지문을 1인칭 구어체로 써서 실패했다.
따옴표 밖 지문은 ${ctx.characterName} 또는 그/그녀를 주어로 한 3인칭 소설체로 다시 쓴다.
"기다리고 있었어", "귀엽네" 같은 대화체 지문을 "기다리고 있었다", "귀엽다고 생각했다"처럼 고친다.
캐릭터다운 반말과 구어체는 큰따옴표 안의 대사에서만 사용한다.` : ""}
${errors.incompleteEnding ? `방금 답변은 마지막 문장이나 대사가 완결되지 않은 채 끝나서 실패했다.
앞부분의 사건, 행동, 위치, 접촉 수위와 대사는 그대로 유지하고, 잘린 마지막 문장만 자연스럽게 완결하라.
분량을 맞추려고 이미 완성된 문단이나 결말을 삭제하지 마라.` : ""}
${errors.regenerationDuplicate ? `방금 답변은 사용자가 폐기하고 재생성을 요청한 기존 답변과 동일해서 실패했다.
폐기된 답변의 문장뿐 아니라 표현만 바꾼 동일 행동, 동일 접촉, 동일 요구도 반복하지 마라.
직전 장면의 확정된 위치와 접촉 상태만 유지하고, 캐릭터의 다음 행동과 대사는 다른 선택으로 새로 전개하라.
이 경우는 제한적 문장 교정이 아니라 새로운 대안 답변을 작성해야 한다.` : ""}
...
검증에 실패한 항목만 최소한으로 수정하고, 실패 항목과 무관한 문장이나 사실은 바꾸지 마라.
`}
...
${ctx.turnPolicy.minChars}~${ctx.turnPolicy.maxChars}자.
...
export function buildComedyRepairPrompt(errors: ReturnType<typeof validateRoleplayOutput>, ctx: CompiledRoleplayContext) {
  ...
  const labels: Record<keyof typeof errors, string> = {
    ...
    providerRefusal: "모델의 사과 또는 요청 거절문만 반환됨",
    degenerateOutput: "역할극 본문으로 사용할 수 없는 초단문이 반환됨",
    unpromptedHandFocus: "손 묘사가 불필요하게 중심이 됨",
    narrationStyleMismatch: "지문이 1인칭 또는 대화체 어미로 작성됨",
    regenerationDuplicate: "재생성 결과가 폐기할 기존 답변의 드립·행동을 의미상 반복함",
    previousResponseDuplicate: "새 답변이 직전 캐릭터 답변의 드립·행동을 의미상 반복함",
    incompleteEnding: "마지막 문장이나 대사가 완결되지 않은 채 잘림",
  }
  const failedLabels = Object.entries(errors)
    .filter(([, failed]) => failed)
...
사용자의 말을 반복하지 말고 의미에만 반응하라.
${ctx.turnPolicy.minChars}~${ctx.turnPolicy.maxChars}자.
출력 한도에 닿기 전에 마지막 대사를 완결하고 반드시 온전한 문장으로 끝내라.
이번 응답 목표: ${ctx.responseGoal}`
}

function resolveRepairPrompt(errors: ReturnType<typeof validateRoleplayOutput>, ctx: CompiledRoleplayContext) {
  return ctx.turnPolicy.comedicPacing ? buildComedyRepairPrompt(errors, ctx) : buildRepairPrompt(errors, ctx)
}
...
- 분량: ${turnPolicy.minChars}~${turnPolicy.maxChars}자, ${turnPolicy.paragraphCount}
...
- 대사와 대사 사이의 지문은 2~3문장 이내로 유지하여 대화 리듬이 끊기지 않게 한다. 긴 내면 해설 대신 하나의 구체적인 동작이나 감각 묘사로 텐션을 잇는다.
...
[몰입 묘사 원칙 (Show, Don't Tell)]
- 캐릭터의 속마음, 의도, 감정 상태를 작가 해설처럼 직접 설명하지 않는다.
- "~하려는 태도였다", "~을 즐기려는 듯했다", "~할 생각이 없어 보였다", "~라는 사실을 잘 알고 있는 눈치였다" 같은 해설형 지문은 쓰지 않는다.
- 대신 구체적인 행동, 표정 변화, 시선 처리, 호흡, 자세, 거리 변화로 독자가 직접 눈치채게 만든다.
- 예시: "여유를 즐기려는 태도였다" → "소파 팔걸이를 톡톡 두드리는 손가락의 리듬은 묘하게 빠르고 집요했다."
- 예시: "정복욕을 부추겼다" → "시선이 이마에서 시작해 콧대, 살짝 벌어진 입술 위를 느릿하게 훑고 내려갔다."
- 분량을 채울 때 속마음 해설을 늘리지 않는다. 대신 영화의 한 장면을 연출하듯 카메라 구도를 좁히고 감각적 디테일을 추가한다.
- 감각적 디테일 예시: 옷감의 질감과 마찰음, 공간의 조명과 그림자, 체온이 닿기 직전의 거리감, 향기, 초침 소리, 숨결이 닿는 감각.
- 감정을 직접 명명하지 말고 신체 반응으로 보여준다: "긴장했다" → "어깨가 미세하게 굳었다", "흥분했다" → "목소리가 반톤 낮아졌다."
...
- "~인 듯했다", "~처럼 보였다", "~인 것 같았다" 같은 추측형 수식어를 남용하지 않는다. 행동과 대사를 통해 단정적으로 보여준다.
- "만족스럽게 가라앉은", "진득한 열기", "노골적으로 오갔다" 같은 장식적이고 관용적인 수식어를 나열하지 않는다. 구체적이고 고유한 묘사를 쓴다.
...
function buildComedySystemPrompt({
  ...
  const { turnPolicy } = compiledContext
  const responseMinChars = turnPolicy.minChars
  const responseMaxChars = turnPolicy.maxChars
  const preferredResponseMinChars = Math.min(responseMaxChars, responseMinChars + 100)
  const preferredResponseMaxChars = Math.max(preferredResponseMinChars, responseMaxChars - 100)
  const minDialogues = profile?.minDialogues ?? COMMON_ROLEPLAY_DIALOGUE_COUNTS.minDialogues
  const preferredDialogues = profile?.preferredDialogues ?? COMMON_ROLEPLAY_DIALOGUE_COUNTS.preferredDialogues
...
- "${userName}"의 새 대사, 새 행동, 새 감정을 만들지 않는다.
- "${userName}"이 웃었다, 물러섰다, 말했다, 생각했다 같은 문장을 쓰지 않는다.
- "${userName}"의 제공된 행동과 대사에만 반응한다.
...
[출력 형식]
- 한국어로만 쓴다.
- 최종 본문은 반드시 ${responseMinChars}~${responseMaxChars}자 범위 안에 들어야 한다.
- 초안은 ${preferredResponseMinChars}~${preferredResponseMaxChars}자를 목표로 작성한다.
- 완결된 대사는 ${minDialogues}~${maxDialogues}개를 쓰고, 특별한 이유가 없으면 ${preferredDialogues}개로 맞춘다.
- 실제로 캐릭터가 입 밖으로 말하는 모든 문장은 예외 없이 큰따옴표로 감싼다. 따옴표 없이 대사를 그냥 줄글로 쓰지 않는다. 인물 이름, 지문, 강조 표현에는 큰따옴표를 쓰지 않는다.
- 대사와 지문은 줄바꿈으로 분리한다.
- 제목, 이름표, 구간명, 설명용 라벨을 붙이지 않는다.
- 출력 한도에 닿기 전에 마지막 대사를 완결하고 온전한 문장으로 끝낸다.
${dialogueCadenceInstructions}
...
[반복 금지]
- 이전 assistant 문장이나 같은 드립을 반복하지 않는다.
...
function buildProfilePromptInstructions(profile?: RoleplayModelProfile) {
  ...
  const modeInstructions: Record<RoleplayModelProfile["outputMode"], string> = {
    chat: "- 3인칭 지문과 대사 중심으로 쓰고 1인칭 속마음 독백은 쓰지 않는다.",
    novel: "- 시간, 장소, 표정, 거리감 같은 3인칭 제한 시점 장면 지문을 더 풍부하게 허용한다.",
    inner_monologue: "- 캐릭터의 속마음은 3인칭 간접 서술로 허용하되 사용자의 생각이나 감정은 쓰지 않는다.",
  }
...
function buildOpenRouterMessages(
  ...
  if (compiledContext?.turnPolicy.autoAdvance) {
    const autoAdvanceContent = formatCompiledUserInputForModel(compiledContext)
    if (lastMessage?.role === "user") {
      lastMessage.content = autoAdvanceContent
    } else {
...
async function callGeminiRoleplay(
  finalMessages: ChatMessages,
  model: ChatModelConfig,
  timeoutStage: GenerationTimeoutStage = "gemini-repair",
) {
  ...
  const generate = async (requestMessages: ChatMessages) => {
    ...
    const response = await (async () => {
      try {
        ...
        }), GEMINI_TIMEOUT_MS, timeoutStage)
      } catch (error) {
...
async function callFreeRoleplay(finalMessages: ChatMessages, model: ChatModelConfig) {
  ...
  return runQueued(async () => {
    ...
    }), POLLINATIONS_TIMEOUT_MS, "pollinations")
...
async function handleRoleplayRulesBypassChat(
  ...
  debugRoleplayContent({ stage: "final", requestId, model: completion.model, content: result })
...
최종 본문은 ${compiledContext.turnPolicy.minChars}~${compiledContext.turnPolicy.maxChars}자, 완결된 대사 ${COMMON_ROLEPLAY_DIALOGUE_COUNTS.minDialogues}~${COMMON_ROLEPLAY_DIALOGUE_COUNTS.maxDialogues}개로 작성하라.`,
    ...
    if (hasClassifiedFailures(classifiedValidation)) {
      ...
      const originalBlockingFailures = getTerminalBlockingFailureKeys(
        originalValidation,
...
${validation.regenerationDuplicate || validation.previousResponseDuplicate
  ...
  : "위 원문을 처음부터 새로 쓰지 말고 실패 항목만 교정한 완성본을 출력하라."}`,
      ...
      debugRoleplayContent({
        ...
        content: retryCompletion.content,
      })
      ...
      } else if (originalBlockingFailures.length > 0) {
...
async function streamGeminiRoleplay({
  ...
  let transientFallbackContent: { model: string; content: string } | undefined
...
${originalProviderContent}`,
    ...
    try {
      ...
      rethrowWithTimeoutStage(fallbackError, timeoutStage)
    }
  ...
  debugRoleplayContent({
    ...
    requestId: runId,
    model: modelName,
    content: originalProviderContent,
  })
  if (transientFallbackContent) {
    debugRoleplayContent({
      stage: "fallback",
      requestId: runId,
      model: transientFallbackContent.model,
      content: transientFallbackContent.content,
    })
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    console.debug("[Gemini RP finish reason]", finishReason)
    console.debug("[Gemini RP safety ratings]", safetyRatings)
    console.debug("[Gemini RP prompt feedback]", promptFeedback)
    console.debug("[Gemini RP candidate parts]", candidatePartDiagnostics)
    console.debug("[RP normalizedInput]", normalizedLatestInput)
    console.debug("[RP contact policy]", {
      physicalContactRequested: compiledContext.latestInput.physicalContactRequested,
      physicalContactPermitted: compiledContext.latestInput.physicalContactPermitted,
  ...
  if (promptBlockOutcome) {
    ...
    repairAttempted = true
    sendPhase("repairing", "빈 응답을 같은 모델로 다시 요청하는 중...")
    const emptyRetryAbortController = new AbortController()
    try {
...
${compiledContext.turnPolicy.minChars}~${compiledContext.turnPolicy.maxChars}자, 대사 ${COMMON_ROLEPLAY_DIALOGUE_COUNTS.minDialogues}~${COMMON_ROLEPLAY_DIALOGUE_COUNTS.maxDialogues}개로 작성하고 ${userName}의 새 행동·대사·감정은 만들지 마라.`,
      ...
      const retryResponse = await withTimeout(ai.models.generateContent({
        model: modelName,
        ...
        config: {
          ...
          abortSignal: emptyRetryAbortController.signal,
        },
      }), EMPTY_VISIBLE_OUTPUT_RETRY_TIMEOUT_MS, "gemini-empty-retry")
  ...
  const initialBlockingFailures = initialValidation && initialClassifiedValidation
    ? getTerminalBlockingFailureKeys(initialValidation, initialClassifiedValidation)
    : []
  if (initialValidation && initialClassifiedValidation) {
    ...
    validationAttempts.push(buildSyntheticValidationAttempt("initial", [
...
초안이 있다면 그 흐름을 버리지 말고 같은 장면의 완성본으로 정리하라.`
  ...
  if (
    ...
    !repairAttempted &&
    !passesTerminalOutputContract(finalValidation, finalClassifiedValidation) &&
...
완결된 대사는 정확히 ${COMMON_ROLEPLAY_DIALOGUE_COUNTS.preferredDialogues}개로 맞추고 마지막 문장을 온전히 끝내라.`
  ...
  if (
    ...
    debugRoleplayJson("[Gemini RP provider fallback decision]", {
      reason: savedContent ? "final-output-contract-remained-after-repair" : "empty-after-repair",
      attemptedModel,
      ...
      terminalBlockingFailures: repairedFinalBlockingFailures,
...
최종 본문은 반드시 ${compiledContext.turnPolicy.minChars}~${compiledContext.turnPolicy.maxChars}자, 완결된 대사 ${COMMON_ROLEPLAY_DIALOGUE_COUNTS.minDialogues}~${COMMON_ROLEPLAY_DIALOGUE_COUNTS.maxDialogues}개(가능하면 ${COMMON_ROLEPLAY_DIALOGUE_COUNTS.preferredDialogues}개)로 작성하라.`,
      ...
      if (
        providerFallbackContent &&
        providerFallbackValidation &&
        providerFallbackClassifiedValidation &&
        passesTerminalOutputContract(providerFallbackValidation, providerFallbackClassifiedValidation)
      ) {
