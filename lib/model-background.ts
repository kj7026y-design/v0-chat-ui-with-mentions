...
export function parseStoryBackground(background = "", characterName = "캐릭터", userName = "사용자"): StoryBackground {
  ...
  return {
    ...
    currentConflict,
    tone,
    consentRule: "상호 동의 기반. 사용자가 접촉을 직접 시작하거나 캐릭터가 먼저 행동해도 된다고 허락하면 신체 접촉을 진행할 수 있다.",
    progressionRule: "신체 접촉과 관계 진전은 이번 턴 입력, 명시적 허락, 현재 장면 상태, contactLevel이 허용할 때 한 단계 진행한다.",
  }
}
...
- 캐릭터 설정에 적극적인 유혹, 노골적인 농담, 주도적인 행동이 명시되어 있으면 이를 거리감, 망설임, 조건 확인만으로 약화하지 않는다.
- 이미 합의된 성인 장면이 구체적으로 진행 중이면 현재 수위와 행동을 이어받아 캐릭터다운 반응을 쓴다. 장면을 플러팅 이전 단계로 되돌리지 않는다.
- 사용자가 접촉/근접/스킨십을 직접 입력하거나 캐릭터가 먼저 행동해도 된다고 허락한 경우, 맥락에 맞는 신체 접촉을 한 단계 진행할 수 있다.
- 신체 접촉은 이번 턴 입력, 명시적 허락, 직전 장면 상태, contactLevel이 허용할 때 사용한다.
